...
# Modified from: https://github.com/SafeRoboticsLab/ISAACS
# --------------------------------------------------------
...
from simulators import (Bicycle5DRefTrajCost, ILQRSpline, LinearPolicy, BaseZeroSumEnv)
...
class BaseMonitor(ABC):
  ...
  def __init__(self, adversary: Callable[[np.ndarray, np.ndarray, Any], np.ndarray]):
    super().__init__()
  ...
  @abstractmethod
  def check(
      self, obs: np.ndarray, agents_action: Optional[Dict[str, np.ndarray]] = None, **kwargs
  ) -> bool:
...
class ValueMonitor(BaseMonitor):
  ...
  def __init__(
      self, adversary: Callable[[np.ndarray, np.ndarray, Any], np.ndarray], critic: Critic,
      value_threshold: float, value_to_be_max: bool
  ):
  ...
  def check(self, obs: np.ndarray, agents_action: Optional[Dict[str, np.ndarray]] = None,
    ...
    action_dict_all = self.add_imaginary_actions(obs=obs, agents_action=agents_action, **kwargs)

    if isinstance(self.critic.action_src, list):
      action = np.concatenate([action_dict_all[key] for key in self.critic.action_src], axis=-1)
    else:
...
class RolloutMonitor(BaseMonitor):
  ...
  def __init__(
      self, adversary: Callable[[np.ndarray, np.ndarray, Any], np.ndarray], env: BaseZeroSumEnv,
      imag_end_criterion: str, imag_steps: int
  ):
    super().__init__(adversary=adversary)
  ...
  def get_nominal_traj(
    ...
    if done:  # The episode is terminated.
      ...
      _traj, result, rollout_info = self.env.simulate_one_trajectory(
          T_rollout=self.imag_steps, end_criterion=self.env.end_criterion, adversary=self.adversary,
  ...
  def check(self, obs: np.ndarray, agents_action: Dict[str, np.ndarray],
            **kwargs) -> Tuple[bool, dict]:
    """
    Args:
        obs (np.ndarray): _description_
    ...
    override_flag, info = self.get_nominal_traj(obs=obs, agents_action=agents_action, **kwargs)
...
class RobustFRSMonitor(RolloutMonitor):
  ...
  def __init__(
    ...
    super().__init__(
        adversary=adversary, env=env, imag_end_criterion=imag_end_criterion, imag_steps=imag_steps
    )
  ...
  def get_linear_policy(
      self, nom_states: np.ndarray, nom_ctrls: np.ndarray, nom_dstbs: np.ndarray
    ...
    K_closed_loop, k_open_loop, _ = self.track_policy.backward_pass(
        c_x=c_x, c_u=c_u, c_xx=c_xx, c_uu=c_uu, c_ux=c_ux, fx=fx, fu=fu, reg=1e-8
    )
  ...
  def get_frs_step(self):
    # def get_frs_step(
    #     self, zono: Zonotope, fx: np.ndarray, fu: np.ndarray, fd: np.ndarray, K_cl: np.ndarray,
    #     input_box: np.ndarray, bias: Optional[np.ndarray] = None
    # ) -> Zonotope:
    #   zono_new = zono.clone()
    #   A = fx + fu@K_cl
    #   B = fd
  ...
  def get_frs(self):
    # def get_frs(self, nom_states: np.ndarray, nom_ctrls: np.ndarray,
    #             nom_dstbs: np.ndarray) -> Tuple[List[Zonotope], np.ndarray, np.ndarray]:
  ...
  def check_single_step_with_verts_sel(self):
    # def check_single_step_with_verts_sel(
    #     self, zono: Zonotope, nom_state: np.ndarray, nom_ctrl: np.ndarray, check_ctrl: bool,
    #     K_closed_loop: Optional[np.ndarray], eps: float = 0.01
    ...
    #   cons_dict: Dict[
    #       str, np.ndarray] = self.env.get_constraints_all(states, dummy_ctrls, dummy_time_indices)
  ...
  def check(self):
    ...
    #       bool: overrides the proposed action (if True).
    #   """
    #   override_flag, nom_info = super().check(obs=obs, agents_action=agents_action, **kwargs)
    ...
    #     raise ValueError(
    #         f"{K_closed_loop.shape[-1]} policies but get " + f"({nom_states.shape[-1]}) states."
    #     )
    #   cfg_linear_policy = SimpleNamespace(device='cpu')
    #   linear_policy = LinearPolicy(
    #       id='linear', nominal_states=nom_states[..., :-1], nominal_controls=nom_ctrls[..., :-1],
    #       K_closed_loop=K_closed_loop, k_open_loop=k_open_loop, cfg=cfg_linear_policy
    #   )
    ...
    #         zono=zonotopes[-1], nom_state=nom_states[..., i], nom_ctrl=nom_ctrls[..., i],
    #         check_ctrl=check_ctrl, K_closed_loop=K
    #     )
