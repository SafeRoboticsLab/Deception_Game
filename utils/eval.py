...
def get_disturbance(dstb_type: str, num_envs: int, key: str,
  ...
  if dstb_type == 'odp':
    ...
    #     odp_values=result_dict['values'], cfg=SimpleNamespace(device='cpu', interp_method='linear')
    # )
    # del result_dict
    ...
    for _ in range(num_envs):
      ...
      dstb_fn_list.append(
          partial(
              adv_dstb, dstb_policy=dstb_policy, use_ctrl=dstb_actor.obs_other_list is not None
          )
      )
    ...
    dstb_fn_list = [
        partial(random_dstb, rng=kwargs['rng'], dstb_range=kwargs['dstb_range'])
        for _ in range(num_envs)
    ]
  else:
