# ----------------------------------------------------------------------------------
# Belief Game | No Belief Robust Baseline | Pretrains Ego Policy
...
from simulators import UberNoBeliefEnv, PrintLogger, save_obj
...
def visualize(
    env: UberNoBeliefEnv, policy: SACDomainRandomization, fig_path: str, vel_list: List[float],
    yaw_list: List[float], num_pts: int = 5, T_rollout: int = 150, end_criterion: str = "failure",
    subfigsz_x: float = 4., subfigsz_y: float = 4., nx: int = 100, ny: int = 100,
    batch_size: int = 512, cmap: str = 'seismic', vmin: float = -.25, vmax: float = .25,
    alpha: float = 0.5, fontsize: int = 16, vel_scatter: bool = False, markersz: int = 40
):
  _xH = 90.
  _yH = 15.
  ...
  trajectories, results = get_trajectories_zs(
      env=env,
      adversary=partial(random_dstb, rng=policy.rng, dstb_range=policy.dstb_range),
      vel_list=vel_list,
      yaw_list=yaw_list,
      num_pts=num_pts,
      T_rollout=T_rollout,
  ...
  for i, vel in enumerate(vel_list):
    ...
    for j, yaw in enumerate(yaw_list):
      ...
      values = get_values(
          env, policy.value, xs, ys, vel, yaw, batch_size=batch_size, fail_value=vmax, xH=_xH,
          yH=_yH
      )
      im = ax.imshow(
          values.T, interpolation='none', extent=env.visual_extent, origin="lower", cmap=cmap,
          vmin=vmin, vmax=vmax, zorder=-1, alpha=alpha
      )
      ...
      for k in range(num_pts):
        ...
        result = results[idx]
        plot_traj(
            ax, trajectory, result, c='g', lw=2., vel_scatter=vel_scatter, zorder=1, s=markersz,
            plot_human=True
        )
...
def main(config_file):
  ...
  if cfg.agent.dyn == "UberNoBelief":
    env_class = UberNoBeliefEnv
  else:
  ...
  print("\n== Solver information ==")
  solver = NaiveRLDomainRandomization(cfg.solver, cfg.train, cfg.arch, cfg.environment)
  policy = solver.policy
  env.agent.init_policy(
      policy_type="NNCS", cfg=SimpleNamespace(device=policy.device), actor=policy.actor.net
  )
  print(
      '\nTotal parameters in actor: {}'.format(
          sum(p.numel() for p in policy.actor.net.parameters() if p.requires_grad)
      )
  )
  print("We want to use: {}, and Agent uses: {}".format(cfg.train.device, policy.device))
  ...
  visualize_callback = partial(
      ...
      T_rollout=cfg.solver.eval_timeout,
      nx=cfg.solver.cmap_res_x,
      ny=cfg.solver.cmap_res_y,
      subfigsz_x=cfg.solver.fig_size_x,
      subfigsz_y=cfg.solver.fig_size_y,
      vmin=-cfg.environment.g_x_fail,
      vmax=cfg.environment.g_x_fail,
      markersz=40,
  )
...
if __name__ == "__main__":
  ...
  parser.add_argument(
      ...
      default=os.path.join("config", "robust_pretrain_ctrl.yaml")
  )
