# ------------------------------------------------------------
# Belief Game with Intent Hypotheses | Pretrains Ego Policy
...
from simulators import BeliefGameWithIntentsEnv, PrintLogger, save_obj
...
def visualize(
    env: BeliefGameWithIntentsEnv, policy: SACDomainRandomization, fig_path: str,
    vel_list: List[float], yaw_list: List[float], num_pts: int = 5, T_rollout: int = 150,
    end_criterion: str = "failure", subfigsz_x: float = 4., subfigsz_y: float = 4., nx: int = 100,
    ny: int = 100, batch_size: int = 512, cmap: str = 'seismic', vmin: float = -.25,
    vmax: float = .25, alpha: float = 0.5, fontsize: int = 16, vel_scatter: bool = False,
    markersz: int = 40
):
  _xH = 90.
  _yH = 15.
...
if __name__ == "__main__":
  ...
  parser.add_argument(
      ...
      default=os.path.join("config", "intent_pretrain_ctrl.yaml")
  )
