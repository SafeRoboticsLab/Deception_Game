...
class BaseMASAC():
  ...
  def build_network(self, verbose: bool = True):
    for idx in range(self.num_critics):
      ...
      critic = Critic(cfg=cfg_critic, cfg_arch=cfg_arch_critic, verbose=verbose, device=self.device)
    ...
    for idx in range(self.num_actors):
      ...
      actor = Actor(cfg=cfg_actor, cfg_arch=cfg_arch_actor, verbose=verbose, device=self.device)
      self.actors[cfg_actor.net_name] = actor
      if not actor.eval:
        self.pg_target_map[actor.pg_target].append(actor.net_name)
    self.action_dim_all: int = np.sum(np.array([x.action_dim for x in self.actors.values()]))
  ...
  def update_actor(self, batch: Batch,
                   timer: int) -> Tuple[Dict[str, Tuple[float, float, float]], Dict[str, bool]]:
  ...
  def update_critic(self, batch: Batch) -> Dict[str, float]:
    ...
    for key, critic in self.critics.items():
      if critic.eval:
        continue
      loss_dict[key] = critic.update(batch, action_nxt_dict, entropy_motives_dict)
    return loss_dict
  ...
  def save(self, step: int, model_folder: str, max_model: Optional[int] = None):
    for critic in self.critics.values():
      if critic.eval:
        continue
