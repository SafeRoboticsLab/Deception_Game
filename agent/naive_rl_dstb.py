...
class NaiveRLDisturbance(NaiveRL):
  ...
  def __init__(self, cfg_agent, cfg_train, cfg_arch, cfg_env, verbose: bool = True):
    ...
    self.warmup_action_range = np.array(cfg_agent.warmup_action_range, dtype=np.float32)
  ...
  def learn(
    ...
    while self.cnt_step <= self.max_steps:
      ...
      for env_idx, (done, info) in enumerate(zip(done_all, info_all)):
        if obs_prev_all[env_idx] is not None:
          ...
          self.store_transition(
              obs_prev_all[env_idx].unsqueeze(0), action_prev_all[env_idx], r_prev_all[env_idx],
              obs_all[[env_idx]], done_prev_all[env_idx], info_prev_all[env_idx]
          )
        if done:
          ...
          self.store_transition(
              obs_all[[env_idx]], action, r_all[env_idx], obs_nxt_all[[env_idx]], done, info
          )
      ...
      if (self.cnt_step >= self.min_steps_b4_opt and self.cnt_opt_period >= self.opt_freq):
        ...
        loss_critic_dict, loss_actor_dict = self.update(self.num_update_per_opt)
        loss = [
            loss_critic_dict['central'], loss_actor_dict['dstb'][0], loss_actor_dict['dstb'][1],
            loss_actor_dict['dstb'][2]
        ]
    ...
    return (train_record, train_progress, violation_record, episode_record, self.pq_top_k)
  ...
  def save(
    ...
    if force_save:
      ...
      _, results, length = venv.simulate_trajectories_zs(
          num_trajectories=num_eval_traj, T_rollout=self.cfg_agent.eval_timeout,
