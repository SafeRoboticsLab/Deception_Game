...
class BaseTraining(ABC):
  ...
  def restore(
    ...
    print('  <= Restore {} with {} updates from {}.'.format(agent_type, step, model_folder))

  @abstractmethod
  def learn(
      self, env: BaseEnv, reset_kwargs: Optional[Dict] = None, action_kwargs: Optional[Dict] = None,
      rollout_step_callback: Optional[Callable] = None,
      rollout_episode_callback: Optional[Callable] = None,
  ...
  def init_learn(self, env: BaseEnv) -> Tuple[VecEnvBase, List[Agent]]:
    ...
    venv = VecEnvBase([copy.deepcopy(env) for _ in range(self.n_envs)], device=self.device)
  ...
  def update(self, num_update_per_opt: int,
    ...
    loss_critic_dict_avg = {
        key: np.mean(np.asarray(value)) for key, value in loss_critic_dict_all.items()
    }
    loss_actor_dict_avg = {
        key: np.mean(np.asarray(value), axis=0) for key, value in loss_actor_dict_all.items()
    }
    return loss_critic_dict_avg, loss_actor_dict_avg

  def check(
      self, env: Union[BaseSingleEnv, BaseZeroSumEnv], venv: VecEnvBase,
      reset_kwargs_list: Optional[List[Dict]] = None, action_kwargs: Optional[Dict] = None,
      rollout_step_callback: Optional[Callable] = None,
      rollout_episode_callback: Optional[Callable] = None,
      visualize_callback: Optional[Callable] = None
  ):
    ...
    save_dict = self.save(
        ...
        action_kwargs_list=action_kwargs, rollout_step_callback=rollout_step_callback,
    ...
    if visualize_callback is not None:
      visualize_callback(env, self.policy, os.path.join(self.figure_folder, f"{self.cnt_step}.png"))
