...
class BaseBlock(ABC):
  ...
  def save(
      self, step: int, model_folder: str, max_model: Optional[int] = None, verbose: bool = True
  ) -> str:
...
class Actor(BaseBlock):
  ...
  def build_network(self, cfg, cfg_arch, verbose: bool = True):
    self.net = GaussianPolicy(
        obs_dim=cfg_arch.obs_dim, mlp_dim=cfg_arch.mlp_dim, action_dim=self.action_dim,
        action_range=self.action_range, append_dim=cfg_arch.append_dim,
        latent_dim=cfg_arch.latent_dim, activation_type=cfg_arch.activation, device=self.device,
        verbose=verbose
    )
    ...
    if hasattr(cfg_arch, "pretrained_path"):
      if cfg_arch.pretrained_path is not None:
        ...
        if verbose:
          print(f"--> [Actor] Loads {self.net_name} from {pretrained_path}.")
  ...
  def load_network(self, pretrained_path, verbose: bool = True):
    self.net.load_state_dict(torch.load(pretrained_path, map_location=self.device))
    if verbose:
      print(f"--> [Actor] Loads {self.net_name} from {pretrained_path}.")
  ...
  def update(self, batch: Batch, critic: Critic) -> Tuple[float, float, float]:
    ...
    if self.obs_other_list is not None:
      other_actions = torch.cat([batch.action[key] for key in self.obs_other_list], dim=-1)
      state = torch.cat([batch.state, other_actions], dim=-1)
    else:
      state = batch.state

    action_sample, log_prob = self.net.sample(state=state, append=append, latent=latent)
    ...
    q_pi_1, q_pi_2 = critic.net(batch.state, action, append=append, latent=latent)
...
class Critic(BaseBlock):
  ...
  def build_network(self, cfg, cfg_arch, verbose: bool = True):
    ...
    self.net = TwinnedQNetwork(
        obs_dim=cfg_arch.obs_dim, mlp_dim=cfg_arch.mlp_dim, action_dim=cfg_arch.action_dim,
    ...
    if hasattr(cfg_arch, "pretrained_path"):
      if cfg_arch.pretrained_path is not None:
        ...
        self.net.load_state_dict(torch.load(pretrained_path, map_location=self.device))
        if verbose:
          print(f"--> [Critic] Loads {self.net_name} from {pretrained_path}.")
  ...
  def update(
    ...
    if isinstance(self.action_src, list):
      action = torch.cat([batch.action[key] for key in self.action_src], dim=-1)
      action_nxt = torch.cat([action_nxt_dict[key] for key in self.action_src], dim=-1)
      tmp = torch.cat([entropy_motives_dict[key] for key in self.action_src], dim=-1)
    ...
    next_q1, next_q2 = self.target(
        batch.non_final_state_nxt, action_nxt, append=non_final_append_nxt, latent=latent
    )

    # Gets Bellman update.
    y = get_bellman_update(
        self.mode, q1.shape[0], self.device, next_q1, next_q2, batch.non_final_mask, batch.reward,
        batch.info['g_x'], batch.info['l_x'], batch.info['binary_cost'], self.gamma,
