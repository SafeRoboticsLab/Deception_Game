...
class NaiveRLDomainRandomization(NaiveRL):
    ...
    while self.cnt_step <= self.max_steps:
      ...
      if self.cnt_step < self.min_steps_b4_opt:
        ctrl_all = self.rng.uniform(
            low=self.warmup_action_range[:, 0], high=self.warmup_action_range[:, 1],
      ...
      action_all = [{'ctrl': ctrl_all[[i]], 'dstb': dstb_all[[i]]} for i in range(self.n_envs)]
      ...
      if (self.cnt_step >= self.min_steps_b4_opt and self.cnt_opt_period >= self.opt_freq):
        ...
        loss = [
            loss_critic_dict['central'], loss_critic_dict['aux'], loss_actor_dict['ctrl'][0],
  ...
  def save(
    ...
    if force_save:
      ...
      adv_fn_list = [
          partial(random_dstb, rng=self.rng, dstb_range=self.policy.dstb_range)
          for _ in range(self.n_envs)
      ]
