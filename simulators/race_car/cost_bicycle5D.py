...
class Bicycle5DCost(BaseSplineCost):
  ...
  def __init__(self, cfg):
    ...
    self.road_barrier_cost = SplineBarrierCost(
        ...
        SplineRoadBoundaryCost(cfg=cfg, buffer=self.buffer)
    )
...
class Bicycle5DSquareConstraint(BaseSplineCost):
  ...
  def get_stage_cost(
    ...
    Returns:
        DeviceArray: scalar.
    """
    state_cost = self.constraint.get_stage_cost(state, ctrl, closest_pt, slope, theta, time_idx)
...
class Bicycle5DReachabilityCost(BaseSplineCost):
  ...
  def get_traj_cost(
    ...
    state_costs = self.constraint.get_cost(state, ctrl, closest_pt, slope, theta, time_indices)
...
class Bicycle5DRefTrajCost(BaseSplineCost):
  ...
  def __init__(self, cfg, ref_traj: np.ndarray, ref_ctrl: Optional[np.ndarray] = None):
    super().__init__(cfg)
