...
class RaceCarDstb5DEnv(BaseZeroSumEnv):
  ...
  def build_obs_rst_space(self, cfg_env, cfg_agent, cfg_cost):
    ...
    # Observation space.
    track_cat = np.concatenate((self.track.track_bound[:2, :], self.track.track_bound[2:, :]),
                               axis=-1)
    x_min, y_min = np.min(track_cat, axis=1)
    x_max, y_max = np.max(track_cat, axis=1)
  ...
  def render_state_cost_map(
      self, ax, nx: int, ny: int, vel: float, yaw: float, delta: float, vmin: float = 0.,
      vmax: float = 20., cmap: str = 'seismic', alpha: float = 0.5, cost_type: str = 'cost',
      xmin: Optional[float] = None, xmax: Optional[float] = None, ymin: Optional[float] = None,
      ymax: Optional[float] = None, time_idx: int = 0
  ) -> np.ndarray:
    if xmin is None:
    ...
    v = self.get_state_cost_map(
        nx=nx, ny=ny, vel=vel, yaw=yaw, delta=delta, cost_type=cost_type, xmin=xmin, xmax=xmax,
        ymin=ymin, ymax=ymax, time_idx=time_idx
    )
    ax.imshow(
