...
class UberNoBeliefEnv(BaseZeroSumEnv):
  """
  ...
  def get_state_cost_map(
      self, nx: int, ny: int, vel: float, yaw: float, delta: float, xH: float, yH: float,
      cost_type: str = 'cost', xmin: Optional[float] = None, xmax: Optional[float] = None,
      ymin: Optional[float] = None, ymax: Optional[float] = None, time_idx: int = 0
  ) -> np.ndarray:
    if xmin is None:
    ...
    v = cost.get_cost(state, ctrl, closest_pt, slope, theta,
                      time_indices=dummy_time_indices).reshape(nx, ny)
    return v

  def render_state_cost_map(
      self, ax, nx: int, ny: int, vel: float, yaw: float, delta: float, xH: float, yH: float,
      vmin: float = 0., vmax: float = 20., cmap: str = 'seismic', alpha: float = 0.5,
      cost_type: str = 'cost', xmin: Optional[float] = None, xmax: Optional[float] = None,
      ymin: Optional[float] = None, ymax: Optional[float] = None, time_idx: int = 0
  ) -> np.ndarray:
    if xmin is None:
    ...
    v = self.get_state_cost_map(
        nx=nx, ny=ny, vel=vel, yaw=yaw, delta=delta, xH=xH, yH=yH, cost_type=cost_type, xmin=xmin,
        xmax=xmax, ymin=ymin, ymax=ymax, time_idx=time_idx
    )
    ax.imshow(
        v.T, interpolation='none', extent=[xmin, xmax, ymin, ymax], origin="lower", cmap=cmap,
